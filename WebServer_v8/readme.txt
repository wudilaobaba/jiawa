...
4.若是请求注册的业务，则通过request获取用户在注册页面上输入的注册信息，并通过RandomAccessFile打开user.dat文件，将该记录写入文件
  每条记录占100字节，其中用户名，密码，昵称占32字节，年龄为int占4字节。
5.创建success.html页面
6.注册完成后，设置跳转至success.html，完成注册流程