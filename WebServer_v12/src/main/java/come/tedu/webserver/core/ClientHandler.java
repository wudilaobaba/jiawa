...
public class ClientHandler implements Runnable {
	...
	public void run() {
		...
		try {
			...
			/**
			 * 判断是否是请求业务
			 * 若ServerContext.getServletName(str)不为null,则说明请求是业务
			 */
			String servletName = ServerContext.getServletName(str);
			if(servletName!=null){
				Class cls = Class.forName(servletName);
				HttpServlet servlet = (HttpServlet)cls.newInstance(); //注意造型，实例化
				servlet.service(request, response);//造型以后可以简化代码
			} else {
